...
class RigRequestHandler : RequestHandler() {
    ...
    override fun load(request: Request, networkPolicy: Int): Result {
        val mainType = getMainType(request.uri)
        val secondType = getSecondType(request.uri)
        val size = getSize(request.uri)

        val generatorParams = createGeneratorParams(mainType, secondType)
        val bitmap = generate(generatorParams, size[0], size[1])
        if (bitmap == null) {
    ...
    private fun generate(generatorParams: GeneratorParams, width: Int, height: Int): Bitmap? {
